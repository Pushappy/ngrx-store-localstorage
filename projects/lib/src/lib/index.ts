    ...
    return (keys as any[]).reduce((acc, curr) => {
        ...
        if (storage !== undefined) {
            ...
            if (stateSlice) {
                ...
                const rehydratedState = deserialize
                    ? deserialize(raw)
                    : raw;
...
export const localStorageSync = (config: LocalStorageConfig) => (reducer: any) => {
    ...
    return function (state: any, action: any) {
        ...
        //
        // This also has to be done on UPADTE_ACTION to not overwrite initial state for feature reducers.
        if (action.type === INIT_ACTION && !state) {
            nextState = reducer(state, action);
        } else if (config.forFeature && action.type === UPDATE_ACTION && !hasDoneFeatureMerge) {
            const featureKey = Object.keys(stateKeys[0])[0]

            if (action.features?.includes(featureKey)) {
                nextState = reducer(state, action);
            }
        }
        else {
        ...
        if (!config.forFeature || !hasDoneFeatureMerge) {
            nextState = mergeReducer(nextState, rehydratedState, action);

            if (config.forFeature) { 
                hasDoneFeatureMerge =  true
            }
        }
